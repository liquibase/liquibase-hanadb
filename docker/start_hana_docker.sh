...
echo "Prior to running this, log in to docker (docker login -u <username), stop and remove the hana_local container, and remove the HXE directory with sudo permissions."
...
docker exec hana_local bash -l -c "hdbsql -u SYSTEM -p L1qu1base_test -i 90 -d HXE 'CREATE USER LIQUIBASE_TEST PASSWORD L1qu1base_test NO FORCE_FIRST_PASSWORD_CHANGE'"
